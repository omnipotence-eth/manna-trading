    ...
    try {
      ...
      if (asterConfig.trading.enable24_7Agents) {
        if (!agentRunnerStatus.isRunning) {
          logger.warn('⚠️ Agent Runner is NOT running - attempting immediate restart (required for 24/7 trading)', {
            context: 'HealthMonitor',
            service: 'AgentRunner',
          ...
          try {
            ...
            if (verifyStatus.isRunning) {
              logger.info('✅ Agent Runner restarted successfully and verified running', {
                context: 'HealthMonitor',
                data: {
                  isRunning: true,
            ...
            logger.error('❌ Failed to restart Agent Runner (CRITICAL - 24/7 trading disabled)', error as Error, {
              context: 'HealthMonitor',
          ...
          if (this.consecutiveFailures > 0) {
            logger.info('✅ Agent Runner recovered - resetting failure counter', {
              context: 'HealthMonitor',
      ...
      if (!balanceConfig || balanceConfig.totalBalance === 0) {
        logger.debug('⚠️ Real Balance Service has no balance data yet (may still be initializing)', {
          context: 'HealthMonitor',
      ...
      try {
        ...
        await deepseekService.chat('ping', undefined, { max_tokens: 5 });
      } catch (error) {
        logger.warn('⚠️ DeepSeek connection check failed', {
          context: 'HealthMonitor',

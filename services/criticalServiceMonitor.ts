...
export class CriticalServiceMonitor {
        ...
        if (this.agentRunnerDownSince === null) {
          ...
          if (downDuration >= this.gracePeriodMs) {
            ...
            if (this.failureMode === 'crash') {
              ...
              logger.error('💀 WOULD CRASH SERVER (but in log-only mode)', {
                context: 'CriticalMonitor',
                failureMode: this.failureMode
              });
            }
            ...
            logger.warn(`⏱️ Agent Runner still down (${(downDuration / 1000).toFixed(0)}s) - ${(remainingGrace / 1000).toFixed(0)}s until server crash`, {
              context: 'CriticalMonitor',
              downDuration: `${(downDuration / 1000).toFixed(0)}s`,
  ...
  private async attemptRecovery(): Promise<void> {
    try {
      logger.info('🔄 Attempting Agent Runner recovery...', {
        context: 'CriticalMonitor'
      });
      ...
      if (status.isRunning) {
        logger.info('✅ Agent Runner recovered successfully!', {
          context: 'CriticalMonitor',
          isRunning: true,
        ...
        logger.error('❌ Agent Runner recovery failed - still not running', {
          context: 'CriticalMonitor',
          isRunning: false
        });
      }
    } catch (error) {
      logger.error('❌ Agent Runner recovery attempt failed', error as Error, {
        context: 'CriticalMonitor'
      });
    }
  }

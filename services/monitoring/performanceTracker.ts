 ...
 * - Proper Sharpe Ratio with risk-free rate
 * - Sortino Ratio for downside risk
 * - Calmar Ratio for drawdown-adjusted returns
 */

import { logger } from '@/lib/logger';
import { calculateRiskMetrics, runMonteCarloSimulation } from '@/lib/advancedMath';
...
export interface PerformanceMetrics {
  ...
  sharpeRatio: number;
  sortinoRatio: number;      // ADDED: Better risk metric
  calmarRatio: number;       // ADDED: Return/MaxDrawdown
  expectedValue: number;     // ADDED: EV per trade
  bestTrade: number;
  worstTrade: number;
...
class PerformanceTracker {
  ...
  async recordTrade(trade: TradePerformance): Promise<void> {
    try {
      ...
      // Ensure table exists (lightweight safeguard for fresh environments)
      await db.execute(`
      ...
      logger.info('[DATA] Trade performance recorded', {
  ...
  async getPerformanceMetrics(days: number = 30): Promise<PerformanceMetrics> {
    try {
      ...
      // Calculate Risk Metrics using proper mathematical formulas
      const pnlPercentages = trades.map((t: any) => t.realizedPnLPercent);
      const sharpeRatio = this.calculateSharpeRatio(pnlPercentages);
      const sortinoRatio = this.calculateSortinoRatio(pnlPercentages);
      
      // Calculate Calmar Ratio: annualized return / max drawdown
      const annualizedReturn = totalPnL * (365 / Math.max(1, days));
  ...
  /**
   * Calculate Sharpe Ratio (annualized) - PROPER FORMULA
   * Sharpe = (Rp - Rf) / σp
   * 
   * @param returns - Array of trade returns as percentages
   * @param riskFreeRate - Annual risk-free rate (default 5%)
   */
  private calculateSharpeRatio(returns: number[], riskFreeRate: number = 5): number {
    if (returns.length < 2) return 0;

    // Convert percentage returns to decimals
    const decimalReturns = returns.map(r => r / 100);
    
    // Use advanced math library for proper calculation
    const metrics = calculateRiskMetrics(decimalReturns, riskFreeRate / 100, 365);
   ...
   * Better for trading systems where we care about losses, not gains volatility
   */
  private calculateSortinoRatio(returns: number[], riskFreeRate: number = 5): number {
    if (returns.length < 2) return 0;
   ...
   */
  async getMonteCarloAnalysis(days: number = 30): Promise<{
    ...
    try {
      ...
      const wins = trades.filter((t: any) => t.realizedPnL > 0);
      const losses = trades.filter((t: any) => t.realizedPnL < 0);
      
      const winRate = trades.length > 0 ? wins.length / trades.length : 0.5;
      const avgWin = wins.length > 0 
        ? wins.reduce((sum: number, t: any) => sum + t.realizedPnLPercent, 0) / wins.length 
        : 3;
      const avgLoss = losses.length > 0 
        ? Math.abs(losses.reduce((sum: number, t: any) => sum + t.realizedPnLPercent, 0) / losses.length)
        : 2;

      // Run Monte Carlo simulation with 1000 paths, 100 trades each
      const monteCarlo = runMonteCarloSimulation(winRate, avgWin, avgLoss, 100, 1000);

      return {
        probabilityOfProfit: monteCarlo.probabilityOfProfit,
        probabilityOfRuin: monteCarlo.probabilityOfRuin,
        medianReturn: monteCarlo.medianReturn * 100,
        worstCase: monteCarlo.percentile5 * 100,
        bestCase: monteCarlo.percentile95 * 100
      };
    } catch (error) {
      logger.error('Monte Carlo analysis failed', error, { context: 'PerformanceTracker' });
      return {
        probabilityOfProfit: 0.5,
        probabilityOfRuin: 0.1,
        medianReturn: 0,

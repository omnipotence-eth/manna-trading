...
// OPTIMIZATION: Use dynamic imports for services to reduce initial load time
// Only logger and config are needed immediately, others can be loaded on demand
...
class StartupService {
  ...
  private async _initialize(): Promise<void> {
    ...
    // STEP 0: Download and preload DeepSeek R1 14B model (NON-BLOCKING - runs in background)
    logger.info('[0/7] Ensuring DeepSeek R1 14B model is downloaded and preloaded...', { context: 'Startup' });
    try {
      const { deepseekService } = await import('@/services/ai/deepseekService');
      
      // Run model download and preload in background (don't block startup)
      // This ensures the model is ready when needed
      deepseekService.ensureModelReady().then(success => {
        if (success) {
          logger.info('[0/7] [OK] DeepSeek R1 14B model is ready and preloaded in RAM!', { 
            context: 'Startup',
            note: 'Model is ready for fast responses'
          });
        } else {
          logger.warn('[0/7] [WARN] DeepSeek R1 model setup incomplete', {
            context: 'Startup',
            note: 'Model may load on first request (30-60 seconds)'
          });
        }
      }).catch(error => {
        logger.warn('[0/7] [WARN] DeepSeek R1 model setup failed (non-critical)', {
          context: 'Startup',
      ...
      // Don't wait for model to load - continue with other services
      // Model will be ready when first request comes in
      logger.info('[0/7] Model download/preload started in background', { context: 'Startup' });
    } catch (error) {
      // CRITICAL CHANGE: Don't fail startup if DeepSeek setup fails
      // Agent Runner can start, workflows will fail but system continues
      logger.warn('[0/7] [WARN] DeepSeek R1 setup error (workflows may fail until model is ready)', {
        context: 'Startup',
        error: error instanceof Error ? error.message : String(error),
    ...
    try {
      ...
      try {
        // OPTIMIZATION: Dynamic import to reduce initial load time
        ...
        wsMarketService.connect().then(() => {
          ...
          if (wsStatus.connected) {
            logger.info('[WebSocket] [OK] Connected! Real-time market data active', {
              context: 'Startup',
        ...
        logger.warn('[0.5/7] [WARN] WebSocket init error (non-critical)', {
          context: 'Startup',
      ...
      try {
        ...
        logger.info('[0.8/7] [OK] Database initialized successfully!', { context: 'Startup' });
      } catch (dbError) {
        logger.warn('[0.8/7] [WARN] Database initialization failed (non-critical)', {
          context: 'Startup',
      ...
      // OPTIMIZATION: Dynamic import (already imported above, but ensure it's available)
      const { positionMonitorService: positionMonitor } = await import('@/services/trading/positionMonitorService');
      ...
      try {
        ...
        // CRITICAL FIX: Use more tokens (at least 20) to ensure model generates actual content
        // DeepSeek R1 sometimes generates tokens but content is empty with very small token counts
        // Also increase timeout to 60 seconds to allow for model loading
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('DeepSeek R1 final verification timeout after 60 seconds')), 60000);
        });
        
        const finalTest = await Promise.race([
          deepseekService.chat('Say hello in one sentence.', undefined, { max_tokens: 30 }),
          timeoutPromise
        ]) as string;
        
        // Validate response has content (not just empty string)
        if (!finalTest || finalTest.trim().length === 0) {
          throw new Error('DeepSeek R1 verification returned empty response - model may not be fully loaded');
        }
        ...
        logger.warn('[5/7] DeepSeek R1 verification failed', {
          ...
          impact: 'Workflows may fail - ensure Ollama is running and model is loaded',
          suggestion: 'Try: ollama run deepseek-r1:14b "test" to verify model works'
        });
      }
      ...
      const { asterConfig: finalCheckConfig } = await import('@/lib/configService');
      if (finalCheckConfig.trading.enable24_7Agents) {
        const { agentRunnerService: finalCheckRunner } = await import('@/services/ai/agentRunnerService');
        const finalRunnerCheck = finalCheckRunner.getStatus();
        ...
        logger.info('[OK] VERIFIED: Agent Runner is running and healthy', {
          context: 'Startup',
      ...
      // OPTIMIZATION: Dynamic import
      const { healthMonitorService } = await import('@/services/monitoring/healthMonitorService');
      ...
      const { asterConfig: monitorConfig } = await import('@/lib/configService');
      if (monitorConfig.trading.enable24_7Agents) {
        logger.info('[7/7] Starting Critical Service Monitor (will crash server if Agent Runner stops)...', { context: 'Startup' });
        
        // OPTIMIZATION: Dynamic import
        const { criticalServiceMonitor } = await import('@/services/monitoring/criticalServiceMonitor');
        ...
        if (!isDevelopment) {
          ...
          logger.warn('[WARN] CRITICAL MONITOR IN CRASH MODE: Server will crash if Agent Runner stops!', {
            context: 'Startup',
            mode: 'crash',
      ...
      // OPTIMIZATION: Dynamic import for final status check
      const { asterConfig: finalStatusConfig } = await import('@/lib/configService');
      const { agentRunnerService: finalStatusRunner } = await import('@/services/ai/agentRunnerService');
      logger.info('Application services initialized successfully', { 
        context: 'Startup',
        data: {
          initialized: true,
          agentRunnerRunning: finalStatusConfig.trading.enable24_7Agents ? finalStatusRunner.getStatus().isRunning : 'disabled',
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      // OPTIMIZATION: Dynamic imports for shutdown
      const { criticalServiceMonitor } = await import('@/services/monitoring/criticalServiceMonitor');
      const { healthMonitorService } = await import('@/services/monitoring/healthMonitorService');
      const { realBalanceService } = await import('@/services/trading/realBalanceService');
      const { positionMonitorService } = await import('@/services/trading/positionMonitorService');
  ...
  async isInitialized(): Promise<boolean> {
    ...
    // OPTIMIZATION: Dynamic import
    try {
      const { asterConfig: checkConfig } = await import('@/lib/configService');
      if (checkConfig.trading.enable24_7Agents) {
        const { agentRunnerService: checkRunner } = await import('@/services/ai/agentRunnerService');
        const isAgentRunnerRunning = checkRunner.getStatus().isRunning;
      ...
      // If dynamic import fails, just return false
      logger.debug('Could not check Agent Runner status', {
        context: 'Startup',
        error: error instanceof Error ? error.message : String(error)
      });
    }
...
const globalForStartup = globalThis as typeof globalThis & {
  __startupService?: StartupService;
};

if (!globalForStartup.__startupService) {
  globalForStartup.__startupService = new StartupService();
}

export const startupService = globalForStartup.__startupService;

...
import { AGENT_PROMPTS, MarketData, AnalystReports, Portfolio, RiskApprovedTrade } from '@/lib/agentPrompts';
...
import { recordTradeEntry, recordTradeRejection } from '@/lib/db';
import { forecasterService } from '@/services/ml/forecasterService';
...
// Import proper types from workflowTypes
import type { 
  WorkflowContext as TypedWorkflowContext,
  TechnicalAnalysisReport,
  FinalDecision as BaseFinalDecision,
  RiskAssessment as BaseRiskAssessment,
  ExecutionPlan as BaseExecutionPlan
} from '@/lib/workflowTypes';

// Extended types for agent coordinator (includes additional fields from DeepSeek responses)
export interface RiskAssessment extends Omit<BaseRiskAssessment, 'reasoning'> {
  approved?: boolean;
...
export interface ExecutionPlan extends BaseExecutionPlan {
  readyToExecute?: boolean;
  reason?: string;
}

export interface FinalDecision extends Omit<BaseFinalDecision, 'reasoning'> {
  reasoning?: string | string[];
  currentPrice?: number;
}

// Use typed context instead of any
export type WorkflowContext = Omit<TypedWorkflowContext, 'riskAssessment' | 'executionPlan' | 'finalDecision'> & {
  // Additional agent-specific context fields
  technicalAnalysis?: TechnicalAnalysisReport & { action?: 'BUY' | 'SELL' | 'HOLD' };
  chiefDecision?: FinalDecision;
  finalDecision?: FinalDecision; // Alias for chiefDecision
  riskAssessment?: RiskAssessment;
  executionPlan?: ExecutionPlan;
  // Additional fields used in workflows
  rlParameters?: any;
  rlApplied?: boolean;
...
export class AgentCoordinator {
  ...
  private initializeAgents(): void {
    const agentConfigs = [
      // Using 14B model for better stability and reasoning
      // 14B model is more stable (fewer empty responses) and has superior reasoning
      // Fits comfortably in 16GB VRAM (RTX 5070 Ti)
  ...
  async startTradingWorkflow(symbol: string): Promise<string> {
    return await this.workflowMutex.runExclusive(async () => {
      ...
      if (existingWorkflow) {
        // Expected behavior - prevent duplicate workflows
        logger.debug('Workflow already running for symbol, returning existing', {
          context: 'AgentCoordinator',
    ...
    setTimeout(() => {
      ...
      if (wf && wf.status === 'running') {
        ...
        if (!hasActiveSteps && wf.steps.every(s => s.status === 'pending')) {
          logger.warn('[WARN] Workflow may be stuck - no steps executed after 100ms', {
            context: 'AgentCoordinator',
            data: {
              workflowId,
              symbol,
      ...
      while (executedSteps < workflow.steps.length && iterations < maxIterations) {
        ...
        for (const step of workflow.steps) {
          if (step.status === 'pending' && this.canExecuteStep(step, workflow)) {
            ...
            logger.info(`[STEP] Executing step: ${step.name} (${step.id})`, {
              context: 'AgentCoordinator',
              data: {
                workflowId,
                symbol: workflow.symbol,
            ...
            try {
              ...
              logger.info(`[OK] Step completed: ${step.name}`, {
                context: 'AgentCoordinator',
                data: {
                  workflowId,
                  symbol: workflow.symbol,
              ...
              const stepDuration = Date.now() - stepStartTime;
              logger.error(`[ERROR] Step failed: ${step.name}`, stepError as Error, {
                context: 'AgentCoordinator',
                data: {
                  workflowId,
                  symbol: workflow.symbol,
        ...
        if (completedSteps === workflow.steps.length) {
          ...
          // CRITICAL FIX: Collect actual error messages from failed steps
          const failedStepDetails = workflow.steps
          ...
          const errorMessages = failedStepDetails.map(s => `${s.stepName}: ${s.error}`).join('; ');
          const workflowError = new Error(`Workflow failed: ${errorMessages}`);
          
          logger.error('Workflow failed with errors', workflowError, {
            context: 'AgentCoordinator',
            workflowId,
            failedSteps,
            completedSteps,
  ...
  private async executeDataGathering(workflow: TradingWorkflow): Promise<any> {
    ...
    logger.info('[DATA] Gathering market data', {
      context: 'AgentCoordinator',
    ...
    try {
      ...
      const { dataIngestionService } = await import('@/services/data/dataIngestionService');
      ...
      logger.info('[OK] Market data gathered successfully', {
        context: 'AgentCoordinator',
        data: { 
      ...
      const duration = Date.now() - startTime;
      logger.error('[ERROR] Data gathering failed', error, {
        context: 'AgentCoordinator',
        data: { 
          workflowId: workflow.id, 
          symbol: workflow.symbol, 
  ...
  private async executeTechnicalAnalysis(workflow: TradingWorkflow): Promise<any> {
    ...
    try {
      ...
      if (!marketData) {
        logger.error('[ERROR] Market data not available for technical analysis', {
          context: 'AgentCoordinator',
          data: {
            workflowId: workflow.id,
            symbol: workflow.symbol,
      ...
      if (!marketData.price || marketData.price <= 0) {
        logger.error('[ERROR] Invalid market data price for technical analysis', {
          context: 'AgentCoordinator',
          data: {
            workflowId: workflow.id,
            symbol: workflow.symbol,
      ...
      if (cachedResult) {
        ...
        logger.info('[OK] Using cached technical analysis', {
          context: 'AgentCoordinator',
          symbol: workflow.symbol,
          cached: true
        });
      ...
      logger.info('[DEEPSEEK] Calling DeepSeek R1 for Technical Analysis', {
        context: 'AgentCoordinator',
        symbol: workflow.symbol,
      ...
      logger.info('[OK] DeepSeek Technical Analysis response received', {
        context: 'AgentCoordinator',
        symbol: workflow.symbol,
        elapsed: `${elapsed}ms`,
  ...
  private async executeChiefDecision(workflow: TradingWorkflow): Promise<any> {
    ...
    try {
      ...
      if (!technicalAnalysis) {
        logger.error('[ERROR] Technical analysis not available for Chief Decision', {
          context: 'AgentCoordinator',
          data: {
            workflowId: workflow.id,
            symbol: workflow.symbol,
      ...
      // CRITICAL FIX: Add retry wrapper for empty responses
      // DeepSeek can return empty responses if model isn't loaded - retry with backoff
      let result: any;
      let attempt = 0;
      const maxAttempts = 3;
      ...
      while (attempt < maxAttempts) {
        try {
          result = await deepseekService.chatWithSystem(systemPrompt, prompt, undefined, {
            format: 'json',
          ...
          const isEmpty = 
            ...
            (typeof result === 'object' && result !== null && 
             ...
             (result.rawContent === '' || result.rawContent === undefined) &&
          ...
          if (attempt < maxAttempts) {
            ...
            logger.warn(`Empty Chief Analyst response, retrying in ${delay}ms (attempt ${attempt}/${maxAttempts})`, {
              context: 'AgentCoordinator',
              symbol: workflow.symbol,
              attempt,
              maxAttempts
          ...
          logger.warn(`Chief Analyst error, retrying in ${delay}ms (attempt ${attempt}/${maxAttempts})`, {
            context: 'AgentCoordinator',
            symbol: workflow.symbol,
      ...
      // CRITICAL FIX: Check for empty response BEFORE parsing
      const isInitialEmptyResponse = 
        result === null || 
        result === undefined ||
        (typeof result === 'string' && result.trim().length === 0) ||
        (typeof result === 'object' && result !== null && 
      ...
      if (isInitialEmptyResponse) {
        logger.warn('DeepSeek returned empty response for Chief Analyst, defaulting to HOLD', {
          context: 'AgentCoordinator',
          symbol: workflow.symbol,
          result: JSON.stringify(result).substring(0, 200),
          note: 'This usually means the model is not loaded or generation failed. Will default to safe HOLD.'
        });
        ...
        return {
          action: 'HOLD',
          confidence: 0.1,
          reasoning: 'Empty response from Chief Analyst - model may not be loaded or generation failed. Defaulting to HOLD for safety.',
          wasHoldOverride: true
        };
      }
      ...
      try {
        ...
        logger.error('Failed to parse Chief Analyst response', parseError as Error, {
          context: 'AgentCoordinator',
          response: typeof result === 'string' ? result.substring(0, 500) : JSON.stringify(result).substring(0, 500),
          resultType: typeof result
        });
        
        // CRITICAL FIX: Try enhanced extraction before throwing error
        try {
          const { deepseekService } = await import('@/services/ai/deepseekService');
          const content = typeof result === 'string' ? result : JSON.stringify(result);
          const extracted = deepseekService.extractStructuredData(content);
          if (extracted && !extracted.error) {
            logger.warn('Using extracted data from malformed Chief Analyst response', {
              context: 'AgentCoordinator',
              data: { 
                action: extracted.action, 
                confidence: extracted.confidence,
                hasAction: !!extracted.action,
                hasConfidence: extracted.confidence !== undefined
              }
            });
            decision = extracted;
          } else {
      ...
      // CRITICAL FIX: Handle missing fields by inferring from reasoning/rawContent
      if (!decision.action || decision.confidence === undefined) {
        // Try to infer from rawContent if available
        if (decision.rawContent && typeof decision.rawContent === 'string' && decision.rawContent.length > 0) {
          const contentLower = decision.rawContent.toLowerCase();
          
          // Infer action from content
          if (!decision.action) {
            if (contentLower.match(/\b(buy|long|purchase|acquire|enter long)\b/)) {
              decision.action = 'BUY';
            } else if (contentLower.match(/\b(sell|short|exit|close|liquidate)\b/)) {
              decision.action = 'SELL';
            } else if (contentLower.match(/\b(hold|wait|pause|no action|stay)\b/)) {
              decision.action = 'HOLD';
            }
          }
          
          // Infer confidence from content (look for percentage or confidence indicators)
          if (decision.confidence === undefined) {
            const confidenceMatch = contentLower.match(/\b(confidence|certainty|probability)[:：]?\s*(\d+\.?\d*)\s*%?/);
            if (confidenceMatch && confidenceMatch[2]) {
              const conf = parseFloat(confidenceMatch[2]);
              decision.confidence = conf > 1 ? conf / 100 : conf;
            } else {
              // Default to low confidence if we can't determine
              decision.confidence = 0.3;
            }
          }
        }
        
        // If still missing, use reasoning text
        if ((!decision.action || decision.confidence === undefined) && decision.reasoning) {
          const reasoningLower = decision.reasoning.toLowerCase();
          
          if (!decision.action) {
            if (reasoningLower.match(/\b(buy|long|purchase|acquire|enter long)\b/)) {
              decision.action = 'BUY';
            } else if (reasoningLower.match(/\b(sell|short|exit|close|liquidate)\b/)) {
              decision.action = 'SELL';
            } else {
              decision.action = 'HOLD';
            }
          ...
          if (decision.confidence === undefined) {
            const confidenceMatch = reasoningLower.match(/\b(confidence|certainty|probability)[:：]?\s*(\d+\.?\d*)\s*%?/);
            if (confidenceMatch && confidenceMatch[2]) {
              const conf = parseFloat(confidenceMatch[2]);
              decision.confidence = conf > 1 ? conf / 100 : conf;
            } else {
              decision.confidence = 0.3; // Default low confidence
            }
          }
        }
        ...
        logger.warn('Inferred missing Chief Analyst fields from content', {
          context: 'AgentCoordinator',
          symbol: workflow.symbol,
          inferredAction: decision.action,
          inferredConfidence: decision.confidence
        });
      }

      // Validate decision structure (after inference)
      if (!decision.action || decision.confidence === undefined) {
        throw new Error(`Chief Analyst response missing required fields: action=${decision.action}, confidence=${decision.confidence}`);
      }
      ...
      if (decision.action === 'HOLD' && technicalAnalysis.action && technicalAnalysis.action !== 'HOLD') {
        ...
        if (techConfidence >= 0.50) {
          logger.warn('[OVERRIDE] HOLD OVERRIDE: Technical Analyst has strong signal - converting to trade', {
            context: 'AgentCoordinator',
            data: {
              symbol: workflow.symbol,
              originalAction: 'HOLD',
  ...
  private async executeRiskAssessment(workflow: TradingWorkflow): Promise<any> {
    ...
    try {
      ...
      if (!finalDecision) {
        logger.error('[ERROR] Chief decision not available for Risk Assessment', {
          context: 'AgentCoordinator',
          data: {
            workflowId: workflow.id,
            symbol: workflow.symbol,
      ...
      // REINFORCEMENT LEARNING PHASE 3: Get liquidation insights from force orders
      // CRITICAL FIX: Initialize with default value to prevent undefined errors
      let liquidationInsights: any = {
        totalLiquidations: 0,
        symbols: [],
        recentLiquidations: [],
        warning: undefined
      ...
      try {
        ...
        const normalizedSymbol = workflow.symbol.replace('/', '');
        const forceOrders = await asterDexService.getForceOrders({
        ...
        if (forceOrders && forceOrders.length > 0) {
          const recentLiquidations = forceOrders.slice(0, 5).map((order: any) => ({
            symbol: order.symbol,
            side: order.positionSide || order.side,
            price: parseFloat(order.price || order.executedQty || 0),
            time: order.time || order.updateTime || Date.now()
          }));
          
          liquidationInsights = {
            totalLiquidations: forceOrders.length,
            symbols: [...new Set(forceOrders.map((o: any) => o.symbol))],
            recentLiquidations: recentLiquidations,
            warning: forceOrders.length >= 3 
              ? `High liquidation count (${forceOrders.length}) - be extra conservative with risk management`
              : undefined
          };
          
          logger.info('Loaded liquidation insights', {
            context: 'AgentCoordinator',
            data: {
              symbol: workflow.symbol,
              totalLiquidations: forceOrders.length,
              recentCount: recentLiquidations.length
            }
          });
        }
      } catch (error) {
        logger.warn('Failed to load liquidation insights (continuing without)', {
          context: 'AgentCoordinator',
          error: error instanceof Error ? error.message : String(error)
        });
        // liquidationInsights already has default value, no need to reassign
      }
      ...
      const prompt = DEEPSEEK_OPTIMIZED_PROMPTS.RISK_MANAGER.assessmentTemplate({
        ...
        liquidationInsights: liquidationInsights // Phase 3: Learn from past liquidations
      ...
      while (attempt < maxAttempts) {
        try {
          ...
          if (attempt < maxAttempts) {
            const delay = Math.min(1000 * Math.pow(2, attempt - 1), 5000); // 1s, 2s, 4s (max 5s)
            logger.warn(`Empty Risk Assessment response, retrying in ${delay}ms (attempt ${attempt}/${maxAttempts})`, {
              context: 'AgentCoordinator',
              symbol: workflow.symbol,
              attempt,
              maxAttempts
          ...
          const delay = Math.min(1000 * Math.pow(2, attempt - 1), 5000);
          logger.warn(`Risk Assessment error, retrying in ${delay}ms (attempt ${attempt}/${maxAttempts})`, {
            context: 'AgentCoordinator',
            symbol: workflow.symbol,
            error: error instanceof Error ? error.message : String(error),
            attempt,
      ...
      logger.info('[OK] DeepSeek Risk Assessment response received', {
        context: 'AgentCoordinator',
        data: {
          symbol: workflow.symbol,
          resultType: typeof result,
      ...
      // CRITICAL FIX: Check for empty response BEFORE parsing
      // DeepSeek can return empty responses when model isn't loaded or generation fails
      const isInitialEmptyResponse = 
        result === null || 
        result === undefined ||
        (typeof result === 'string' && result.trim().length === 0) ||
      ...
      if (isInitialEmptyResponse) {
        logger.warn('DeepSeek returned empty response for Risk Assessment, defaulting to rejection', {
          context: 'AgentCoordinator',
          symbol: workflow.symbol,
          result: JSON.stringify(result).substring(0, 200),
          note: 'This usually means the model is not loaded or generation failed. Will default to safe rejection.'
        });
        
        // Return safe default rejection
        return {
          approved: false,
          action: 'HOLD',
          reasoning: 'Empty response from Risk Manager - model may not be loaded or generation failed. Defaulting to rejection for safety.',
          confidence: 0.1
        };
      }
      ...
      try {
        ...
        // CRITICAL FIX: Handle rawContent containing JSON in code blocks
        // When DeepSeek parsing fails, it puts the original content in rawContent
        if (riskAssessment.rawContent && typeof riskAssessment.rawContent === 'string') {
          const rawContent = riskAssessment.rawContent;
          ...
          if (jsonMatch && jsonMatch[1]) {
            try {
              ...
              logger.info('Extracted JSON from rawContent code block', {
                context: 'AgentCoordinator',
                data: { approved: extractedJson.approved, action: extractedJson.action }
              });
              // Merge extracted JSON into riskAssessment
              riskAssessment = { ...riskAssessment, ...extractedJson };
            } catch (extractError) {
              logger.warn('Failed to parse JSON from rawContent code block', {
                context: 'AgentCoordinator',
                data: { rawContent: rawContent.substring(0, 200) }
              });
            }
          }
          
          // CRITICAL FIX: If still missing approved, try to extract from rawContent text
          if (riskAssessment.approved === undefined && rawContent.length > 0) {
            const contentLower = rawContent.toLowerCase();
            // Look for explicit approval/rejection statements
            if (contentLower.match(/\b(approved?|accept|yes|go|proceed|execute|take the trade|enter)\b/)) {
              riskAssessment.approved = true;
              logger.info('Inferred approved=true from rawContent text', {
                context: 'AgentCoordinator',
                symbol: workflow.symbol
              });
              ...
              logger.info('Inferred approved=false from rawContent text', {
                context: 'AgentCoordinator',
                symbol: workflow.symbol
              });
            }
        ...
        if (riskAssessment.riskAssessment && typeof riskAssessment.riskAssessment === 'object') {
          ...
          if (nested.finalDecision !== undefined) {
            riskAssessment.approved = nested.finalDecision === 'APPROVE' || nested.finalDecision === 'APPROVED' || nested.finalDecision === true;
            // Only set action to BUY if approved, otherwise keep existing or set to HOLD
            if (riskAssessment.approved && nested.action) {
              riskAssessment.action = nested.action;
            } else if (!riskAssessment.approved) {
              riskAssessment.action = nested.action || 'HOLD';
            }
            riskAssessment.reasoning = nested.reasoning || riskAssessment.reasoning || 'Derived from nested response';
            
            // Extract position sizing if available - CRITICAL: Must be > 0 if approved
            if (nested.positionSizing) {
              const extractedSize = nested.positionSizing.sizeUSD || nested.positionSizing.size;
              if (extractedSize && typeof extractedSize === 'number' && extractedSize > 0) {
                riskAssessment.positionSize = extractedSize;
              }
            } else if (nested.positionSize) {
              // Also check for direct positionSize field
              if (typeof nested.positionSize === 'number' && nested.positionSize > 0) {
                riskAssessment.positionSize = nested.positionSize;
              }
            }
            
            if (nested.leverage) {
              if (typeof nested.leverage === 'number') {
                riskAssessment.leverage = nested.leverage;
              } else {
                riskAssessment.leverage = nested.leverage.recommended || nested.leverage.value || 15;
              }
            }
            
            if (nested.stopLoss !== undefined) {
              // Handle both percentage and price formats
              if (typeof nested.stopLoss === 'number' && nested.stopLoss > 0) {
                riskAssessment.stopLoss = nested.stopLoss;
              } else if (nested.stopLoss && typeof nested.stopLoss === 'object') {
                riskAssessment.stopLoss = nested.stopLoss.percent || nested.stopLoss.percentage || nested.stopLoss.price;
              }
            }
            
            if (nested.takeProfit !== undefined) {
              // Handle both percentage and price formats
              if (typeof nested.takeProfit === 'number' && nested.takeProfit > 0) {
                riskAssessment.takeProfit = nested.takeProfit;
              } else if (nested.takeProfit && typeof nested.takeProfit === 'object') {
                riskAssessment.takeProfit = nested.takeProfit.percent || nested.takeProfit.percentage || nested.takeProfit.price;
              }
            }
            
            // Extract risk/reward ratio if available
            if (nested.riskRewardRatio !== undefined) {
              riskAssessment.riskRewardRatio = nested.riskRewardRatio;
            } else if (nested.riskReward !== undefined) {
              riskAssessment.riskRewardRatio = nested.riskReward;
            }
            
            // Extract risk percentage if available
            if (nested.riskPercentage !== undefined) {
              riskAssessment.riskPercentage = nested.riskPercentage;
            } else if (nested.riskPercent !== undefined) {
              riskAssessment.riskPercentage = nested.riskPercent;
            }
          }
        }
      } catch (parseError) {
      ...
      // CRITICAL FIX: Handle empty or invalid responses gracefully
      // If response is empty or only contains rawContent/reasoning, default to rejection
      const isPostParseEmptyResponse = 
        !riskAssessment || 
        (Object.keys(riskAssessment).length <= 2 && 
         (riskAssessment.rawContent === '' || riskAssessment.rawContent === undefined) &&
         (!riskAssessment.approved && !riskAssessment.action && !riskAssessment.reasoning));
      
      if (isPostParseEmptyResponse) {
        logger.warn('Empty or invalid Risk Manager response, defaulting to rejection', {
          context: 'AgentCoordinator',
          response: riskAssessment,
          symbol: workflow.symbol,
          keys: riskAssessment ? Object.keys(riskAssessment) : [],
          rawContent: riskAssessment?.rawContent,
          hasApproved: riskAssessment?.approved !== undefined,
          hasAction: riskAssessment?.action !== undefined
        });
        riskAssessment = {
          approved: false,
          action: 'HOLD',
          reasoning: 'Empty or invalid response from Risk Manager - defaulting to rejection for safety',
          confidence: 0.1
        };
      }
      ...
      // CRITICAL FIX: If approved is still undefined after all extraction attempts, default to false
      if (typeof riskAssessment.approved !== 'boolean') {
        logger.warn('Approved field is not boolean, defaulting to false', {
          context: 'AgentCoordinator',
          approvedType: typeof riskAssessment.approved,
          approvedValue: riskAssessment.approved,
          symbol: workflow.symbol
        ...
        riskAssessment.approved = false;
        riskAssessment.action = riskAssessment.action || 'HOLD';
        riskAssessment.reasoning = riskAssessment.reasoning || 'Could not determine approval status - defaulting to rejection';
      }
      
      // CRITICAL FIX: When trade is rejected (approved: false), action and positionSize can be empty/zero
      // Only require these fields when the trade is approved AND action is BUY/SELL (not HOLD)
      if (riskAssessment.approved === true) {
        // CRITICAL FIX: If action is HOLD, treat it as approved=false (can't approve a HOLD)
        // This handles cases where AI returns approved:true with action:HOLD (which doesn't make sense)
        const isHoldAction = !riskAssessment.action || riskAssessment.action === 'HOLD';
        
        if (isHoldAction) {
          logger.warn('Risk Manager returned approved:true with action:HOLD - treating as rejection', {
            context: 'AgentCoordinator',
            symbol: workflow.symbol,
            note: 'Cannot approve a HOLD action - converting to approved:false'
          });
          riskAssessment.approved = false;
          riskAssessment.action = 'HOLD';
          riskAssessment.positionSize = 0;
        } else {
          // Only validate action for BUY/SELL trades
          if (riskAssessment.action !== 'BUY' && riskAssessment.action !== 'SELL') {
            missingFields.push(`action (got ${typeof riskAssessment.action}: ${riskAssessment.action}) - must be BUY or SELL for approved trades`);
          }
          
          // CRITICAL FIX: Only require positionSize for BUY/SELL trades, not HOLD
          // If positionSize is 0, undefined, or not a number, we need to calculate it or reject
          if (typeof riskAssessment.positionSize !== 'number' || riskAssessment.positionSize <= 0 || !isFinite(riskAssessment.positionSize)) {
            // Try to calculate position size from risk percentage if available
            if (typeof riskAssessment.riskPercentage === 'number' && riskAssessment.riskPercentage > 0 && currentPrice > 0) {
              const riskAmount = balance * (riskAssessment.riskPercentage / 100);
              const stopDistance = riskAssessment.stopLoss 
                ? Math.abs(currentPrice - riskAssessment.stopLoss) / currentPrice
                : 0.04; // Default 4% stop if not provided
              
              if (stopDistance > 0) {
                riskAssessment.positionSize = riskAmount / (stopDistance * currentPrice);
                logger.info('Calculated positionSize from riskPercentage', {
                  context: 'AgentCoordinator',
                  data: {
                    riskPercentage: riskAssessment.riskPercentage,
                    calculatedSize: riskAssessment.positionSize,
                    stopDistance: (stopDistance * 100).toFixed(2) + '%'
                  }
                });
              } else {
                missingFields.push(`positionSize (got ${typeof riskAssessment.positionSize}: ${riskAssessment.positionSize}) - cannot calculate from riskPercentage without stopLoss`);
              }
            } else {
              missingFields.push(`positionSize (got ${typeof riskAssessment.positionSize}: ${riskAssessment.positionSize}) - required for BUY/SELL trades`);
            }
          }
        }
        
        // Validate stopLoss and takeProfit are valid numbers if provided
        if (riskAssessment.stopLoss !== undefined && (typeof riskAssessment.stopLoss !== 'number' || !isFinite(riskAssessment.stopLoss) || riskAssessment.stopLoss <= 0)) {
          logger.warn('Invalid stopLoss in risk assessment, will calculate from ATR', {
            context: 'AgentCoordinator',
            data: { stopLoss: riskAssessment.stopLoss }
          });
          riskAssessment.stopLoss = undefined; // Will be calculated later
        }
        
        if (riskAssessment.takeProfit !== undefined && (typeof riskAssessment.takeProfit !== 'number' || !isFinite(riskAssessment.takeProfit) || riskAssessment.takeProfit <= 0)) {
          logger.warn('Invalid takeProfit in risk assessment, will calculate from R:R', {
            context: 'AgentCoordinator',
            data: { takeProfit: riskAssessment.takeProfit }
          });
          riskAssessment.takeProfit = undefined; // Will be calculated later
        }
      }
      ...
      if (missingFields.length > 0) {
        const validationError = new Error(`Risk Manager response missing required fields: ${missingFields.join(', ')}`);
        logger.error('Risk Manager response validation failed', validationError, {
          context: 'AgentCoordinator',
          data: {
            missingFields,
            receivedFields: Object.keys(riskAssessment),
            response: JSON.stringify(riskAssessment).substring(0, 1000),
            symbol: workflow.symbol,
            workflowId: workflow.id
          }
        });
      ...
      // CRITICAL FIX: Check Chief Analyst confidence vs threshold BEFORE Risk Manager
      // This saves DeepSeek API calls and ensures we only assess high-confidence trades
      const { asterConfig } = await import('@/lib/configService');
      ...
      if (chiefConfidence < threshold) {
        logger.warn(`Trade REJECTED: Chief Analyst confidence below threshold (before Risk Manager)`, {
          context: 'AgentCoordinator',
          data: {
            symbol: workflow.symbol,
            ...
            balance,
            note: 'Rejected early to save API calls'
          }
        });
        // Return early rejection without calling Risk Manager
        return {
          approved: false,
          action: 'HOLD',
          reasoning: `Trade rejected: Chief Analyst confidence ${(chiefConfidence*100).toFixed(0)}% below required ${(threshold*100).toFixed(0)}% threshold`,
          positionSize: 0,
          leverage: 15,
          stopLoss: 0,
          takeProfit: 0
      ...
      logger.info(`[OK] Confidence check PASSED: Chief Analyst ${(chiefConfidence*100).toFixed(0)}% >= Threshold ${(threshold*100).toFixed(0)}%`, {
        context: 'AgentCoordinator',
        data: { symbol: workflow.symbol, chiefConfidence, threshold }
      });

      // CRITICAL FIX: Apply RL optimization FIRST, then forecaster trim
      // This ensures RL recommendations are properly applied before any trimming
      // RL OPTIMIZATION: apply RL-suggested params to sizing and RR caps (safe clamps)
      if (riskAssessment.approved) {
        try {
          const { rlParameterOptimizer } = await import('@/services/ml/rlParameterOptimizer');
          const diag = rlParameterOptimizer.getDiagnostics();
          // OPTIMIZATION: Run RL optimization (per-symbol learning happens in learnFromTrade)
          // Note: runOptimization doesn't take symbol - per-symbol Q-tables are updated via learnFromTrade
          const rlResult = await rlParameterOptimizer.runOptimization({ 
            maxSteps: 2
          });
          const rlParams = rlResult?.params;
          const currentPrice = workflow.context.finalDecision?.currentPrice || workflow.context.marketData?.price || 0;
          
          if (rlParams && currentPrice > 0) {
            // Apply stop/tp if missing, using RL percent bands
            if (!riskAssessment.stopLoss) {
              const slPct = Math.max(rlParams.stopLossPercent, 0.5) / 100;
              riskAssessment.stopLoss = currentPrice * (riskAssessment.action === 'BUY' ? (1 - slPct) : (1 + slPct));
            }
            if (!riskAssessment.takeProfit) {
              const tpPct = Math.max(rlParams.takeProfitPercent, rlParams.stopLossPercent * rlParams.rrRatio) / 100;
              riskAssessment.takeProfit = currentPrice * (riskAssessment.action === 'BUY' ? (1 + tpPct) : (1 - tpPct));
            }
            riskAssessment.riskRewardRatio = rlParams.rrRatio;

            // Cap position size to RL risk percent; only shrink, never enlarge
            const stopDistance = Math.abs(currentPrice - (riskAssessment.stopLoss || currentPrice)) / currentPrice;
            if (stopDistance > 0) {
              const rlRiskPct = Math.max(rlParams.positionSizePercent, 0.5) / 100;
              const rlSize = (balance * rlRiskPct) / (stopDistance * currentPrice);
              if (!riskAssessment.positionSize || rlSize < riskAssessment.positionSize) {
                riskAssessment.positionSize = Math.max(0.001, rlSize);
              }
            }

            // Tag workflow and assessment for downstream logging
            workflow.context.rlParameters = rlParams;
            riskAssessment.rlParameters = rlParams;
            workflow.context.rlApplied = true;

            logger.info('Applied RL optimization parameters', {
              context: 'AgentCoordinator',
              data: { symbol: workflow.symbol, rlParams, rewardsTracked: diag?.rewardsTracked }
            });
          } else {
            workflow.context.rlApplied = false;
            logger.debug('RL optimization skipped (no params)', {
              context: 'AgentCoordinator',
              data: { symbol: workflow.symbol, rewardsTracked: diag?.rewardsTracked || 0 }
            });
          }
        } catch (rlErr) {
          workflow.context.rlApplied = false;
          logger.debug('RL optimization skipped (non-blocking)', {
            context: 'AgentCoordinator',
            data: { error: rlErr instanceof Error ? rlErr.message : String(rlErr) }
          });
        }
      }
      
      // OPTIMIZATION: Apply Kelly Criterion as position size cap/validation
      // Provides mathematically optimal sizing based on win probability and risk/reward
      if (riskAssessment.approved && riskAssessment.positionSize && currentPrice > 0) {
        try {
          const { calculateFullKelly } = await import('@/lib/kellyCriterion');
          
          // Calculate win probability from confidence (conservative estimate)
          // Use finalDecision confidence as base, adjust for risk/reward
          const baseWinProb = Math.max(0.5, Math.min(0.9, finalDecision.confidence));
          
          // Get risk/reward ratio
          const rr = riskAssessment.riskRewardRatio || 1;
          
          // Calculate win/loss ratio from risk/reward
          // If RR = 2:1, then win/loss ratio = 2
          const winLossRatio = Math.max(1.1, Math.min(5, rr));
          
          // Calculate Full Kelly: f* = (bp - q) / b
          // where b = win/loss ratio, p = win probability, q = loss probability
          const fullKelly = calculateFullKelly(baseWinProb, winLossRatio);
          
          // Use fractional Kelly (15% of full Kelly for safety, matching system standard)
          const fractionalKelly = fullKelly * 0.15;
          
          // Convert to position size in USD
          const kellyOptimalSize = balance * fractionalKelly;
          
          // Use Kelly as a cap - never exceed Kelly-optimal size
          if (riskAssessment.positionSize > kellyOptimalSize) {
            const beforeCap = riskAssessment.positionSize;
            riskAssessment.positionSize = kellyOptimalSize;
            
            logger.info('Kelly Criterion capped position size', {
              context: 'AgentCoordinator',
              data: {
                symbol: workflow.symbol,
                beforeCap: beforeCap.toFixed(2),
                afterCap: kellyOptimalSize.toFixed(2),
                fullKelly: (fullKelly * 100).toFixed(2) + '%',
                fractionalKelly: (fractionalKelly * 100).toFixed(2) + '%',
                winProb: (baseWinProb * 100).toFixed(1) + '%',
                winLossRatio: winLossRatio.toFixed(2),
                note: 'Position size capped to Kelly-optimal for risk management'
              }
            });
            
            riskAssessment.reasoning = `${riskAssessment.reasoning || ''} | Size capped to Kelly-optimal (${(fractionalKelly * 100).toFixed(1)}% of balance)`;
          } else {
            logger.debug('Position size within Kelly-optimal bounds', {
              context: 'AgentCoordinator',
              data: {
                symbol: workflow.symbol,
                positionSize: riskAssessment.positionSize.toFixed(2),
                kellyOptimal: kellyOptimalSize.toFixed(2),
                utilization: ((riskAssessment.positionSize / kellyOptimalSize) * 100).toFixed(1) + '%'
              }
            });
          }
          
          // Store Kelly calculation in workflow context for analysis
          workflow.context.kellyCalculation = {
            fullKelly: fullKelly,
            fractionalKelly: fractionalKelly,
            kellyOptimalSize: kellyOptimalSize,
            winProbability: baseWinProb,
            winLossRatio: winLossRatio,
            capped: riskAssessment.positionSize === kellyOptimalSize
          };
        } catch (kellyErr) {
          // Non-blocking - Kelly is a validation tool, not required
          logger.debug('Kelly Criterion calculation skipped (non-blocking)', {
            context: 'AgentCoordinator',
            data: { 
              symbol: workflow.symbol, 
              error: kellyErr instanceof Error ? kellyErr.message : String(kellyErr) 
            }
          });
        }
      }
      ...
      if (riskAssessment.approved) {
        const { positionMonitorService } = await import('@/services/trading/positionMonitorService');
        ...
        // MICROSTRUCTURE GUARDRAILS: reject if spread too wide or book too thin
        const microSig = wsMarketService.getMicrostructureSignal(workflow.symbol);
        if (microSig) {
          const totalNotional = microSig.bidNotional + microSig.askNotional;
          if (microSig.spreadPct > 0.25 || totalNotional < 20000) {
            riskAssessment.approved = false;
            riskAssessment.reasoning = `Trade rejected: Spread/liquidity guard. Spread ${microSig.spreadPct.toFixed(3)}%, depth $${(totalNotional/1000).toFixed(1)}k`;
          }
        }

        // SLOT ENFORCEMENT: Only 1 micro + 1 macro simultaneously
        const classifyStyle = (entry: number, stop: number, take: number): 'MICRO' | 'MACRO' => {
          const entryPrice = Math.max(entry, 1e-9);
          const slPct = Math.abs(stop - entryPrice) / entryPrice * 100;
          const tpPct = Math.abs(take - entryPrice) / entryPrice * 100;
          if (slPct <= 1.2 && tpPct <= 3.5) return 'MICRO';
          return 'MACRO';
        };

        const openStyles = openPositions.reduce(
          (acc, p) => {
            const style = classifyStyle(p.entryPrice, p.stopLoss, p.takeProfit);
            if (style === 'MICRO') acc.micro++;
            else acc.macro++;
            return acc;
          },
          { micro: 0, macro: 0 }
        ...
        const entryForStyle = workflow.context.finalDecision?.currentPrice || workflow.context.marketData?.price || 0;
        const newStyle = classifyStyle(
          entryForStyle || 1,
          riskAssessment.stopLoss || entryForStyle,
          riskAssessment.takeProfit || entryForStyle
        );

        if ((newStyle === 'MICRO' && openStyles.micro >= 1) || (newStyle === 'MACRO' && openStyles.macro >= 1)) {
          riskAssessment.approved = false;
          riskAssessment.reasoning = `Trade rejected: ${newStyle} slot already filled (micro ${openStyles.micro}, macro ${openStyles.macro})`;
        }
        ...
        try {
          ...
          logger.info(`[OPTIMIZE] Leverage optimized for ${workflow.symbol}`, {
            context: 'AgentCoordinator',
            data: {
              symbol: workflow.symbol,
        ...
        const { problematicCoinDetector } = await import('@/services/trading/problematicCoinDetector');
        ...
        // [QUALITY GATE] COMPOUND GROWTH QUALITY GATE: Only take A+ setups for maximum growth
        // This is the KEY to turning $60 into wealth - be SELECTIVE
        if (riskAssessment.approved) {
          try {
            const { compoundGrowthEngine } = await import('@/services/trading/compoundGrowthEngine');
            await compoundGrowthEngine.initialize(balance);
            
            // Calculate spread and liquidity from market data
            const marketData = workflow.context.marketData;
            const spread = marketData?.bidAskSpread || marketData?.spread || 0.3;
            const liquidity = marketData?.volume24h || marketData?.volume || 0;
            const score = workflow.context.analysisResult?.score || 65;
            
            const qualityCheck = await compoundGrowthEngine.shouldTakeTrade({
              confidence: finalDecision.confidence,
              riskReward: riskAssessment.riskRewardRatio || 0,
              spread,
              liquidity,
              score,
              hasMultiTimeframe: !!workflow.context.analysisResult?.multiTimeframeConfirmed,
              hasVolumeConfirmation: (marketData?.volumeRatio || 0) > 1.5
            });
            
            if (!qualityCheck.shouldTrade) {
              logger.info(`[REJECT] QUALITY GATE REJECTED: ${qualityCheck.reason}`, {
                context: 'AgentCoordinator',
                data: {
                  symbol: workflow.symbol,
                  qualityScore: qualityCheck.qualityScore,
                  improvements: qualityCheck.improvements.slice(0, 3),
                  gate: compoundGrowthEngine.getQualityGate().name
                }
              });
              riskAssessment.approved = false;
              riskAssessment.reasoning = `Quality gate: ${qualityCheck.improvements[0]} (Score: ${qualityCheck.qualityScore}/100)`;
            } else {
              logger.info(`[OK] QUALITY GATE PASSED: A+ setup detected!`, {
                context: 'AgentCoordinator',
                data: {
                  symbol: workflow.symbol,
                  qualityScore: qualityCheck.qualityScore,
            ...
            // Non-critical - continue with trade if quality check fails
            logger.debug('Quality gate check failed, continuing with trade', {
              context: 'AgentCoordinator',
      ...
      if (!riskAssessment.approved && finalDecision.action !== 'HOLD' && finalDecision.confidence >= 0.50) {
        ...
        if (!isCriticalRejection) {
          logger.warn('[OVERRIDE] FORCE APPROVAL: Chief Analyst has strong signal, overriding Risk Manager rejection', {
            context: 'AgentCoordinator',
            data: {
              symbol: workflow.symbol,
          ...
          // PERFORMANCE-BASED SIZING: Get optimal position size based on recent wins/losses
          // After consecutive wins, we size up. After losses, we scale back.
          let riskPercent = 0.02; // Default 2%
          let optimalLeverage = 15; // Default
          let sizingReason = '';
          ...
          try {
            ...
            riskPercent = sizing.positionSizePercent / 100; // Convert % to decimal
            optimalLeverage = sizing.maxLeverage;
            sizingReason = ` | ${sizing.tier.name} tier (${sizing.positionSizePercent}% risk) - ${sizing.reasoning}`;
            
            if (sizing.shouldSizeUp) {
              logger.info('[SIZE] SIZING UP after consecutive wins!', {
                context: 'AgentCoordinator',
                data: {
                  symbol: workflow.symbol,
                  tier: sizing.tier.name,
                  positionSize: sizing.positionSizePercent + '%',
                  maxLeverage: sizing.maxLeverage
                }
              });
            }
          } catch (sizerError) {
            // Fallback to default if sizer fails
            logger.debug('Using default sizing (sizer not available)', {
              context: 'AgentCoordinator'
            });
          }
          
          const stopDistance = Math.abs(currentPrice - stopLossPrice) / currentPrice;
          let safePositionSize = (balance * riskPercent) / (stopDistance * currentPrice);
          
          // CRITICAL: Validate position size against exchange filters BEFORE setting
          // Per API docs: https://github.com/asterdex/api-docs/blob/master/aster-finance-futures-api.md
          try {
            const { asterDexService } = await import('@/services/exchange/asterDexService');
            
            // Get symbol filters (MIN_NOTIONAL, LOT_SIZE, PERCENT_PRICE)
            const normalizedSymbol = workflow.symbol.replace('/', '').toUpperCase();
            ...
            if (symbolData) {
              ...
              // Get MIN_NOTIONAL requirement
              const minNotional = minNotionalFilter ? parseFloat(minNotionalFilter.minNotional || '5.0') : 5.0;
              
              // Get LOT_SIZE requirements
              const minQty = lotSizeFilter ? parseFloat(lotSizeFilter.minQty || '0') : 0;
              const stepSize = lotSizeFilter ? parseFloat(lotSizeFilter.stepSize || '0') : 0;
              const maxQty = lotSizeFilter ? parseFloat(lotSizeFilter.maxQty || '0') : 0;
              
              // Round position size to stepSize
              let adjustedSize = safePositionSize;
              if (stepSize > 0) {
                adjustedSize = Math.floor(adjustedSize / stepSize) * stepSize;
                // Ensure it's at least minQty
                if (adjustedSize < minQty && minQty > 0) {
                  adjustedSize = Math.ceil(minQty / stepSize) * stepSize;
                }
              }
              
              // Cap to maxQty if specified
              if (maxQty > 0 && adjustedSize > maxQty) {
                adjustedSize = Math.floor(maxQty / stepSize) * stepSize;
              }
              
              // Validate MIN_NOTIONAL
              const positionValue = adjustedSize * currentPrice;
              if (positionValue < minNotional) {
                // Round up to meet MIN_NOTIONAL
                const minSize = Math.ceil(minNotional / currentPrice / (stepSize || 0.0001)) * (stepSize || 0.0001);
                adjustedSize = Math.max(adjustedSize, minSize);
                
                logger.warn('Position size adjusted to meet MIN_NOTIONAL', {
                  context: 'AgentCoordinator',
                  data: {
              ...
              const maxLeverage = await asterDexService.getMaxLeverage(workflow.symbol);
              if (optimalLeverage > maxLeverage) {
                logger.warn('Leverage capped to symbol maximum', {
                  context: 'AgentCoordinator',
                  data: {
                    requested: optimalLeverage,
              ...
              // CRITICAL: Check ADL Quantile (liquidation risk)
              // Per API docs: Higher ADL quantile = higher liquidation risk
              try {
                const adlQuantile = await asterDexService.getADLQuantile(workflow.symbol);
                if (adlQuantile && adlQuantile.adlQuantile > 0.7) {
                  // High liquidation risk - reduce position size by 50%
                  adjustedSize = adjustedSize * 0.5;
                  logger.warn('Position size reduced due to high ADL quantile (liquidation risk)', {
                    context: 'AgentCoordinator',
                    data: {
                      symbol: workflow.symbol,
                      adlQuantile: adlQuantile.adlQuantile,
                      originalSize: safePositionSize,
                      adjustedSize,
                      note: 'ADL quantile > 0.7 indicates high liquidation risk'
                    }
                  });
                } else if (adlQuantile && adlQuantile.adlQuantile < 0.3) {
                  // Low liquidation risk - can increase position size by 10%
                  adjustedSize = adjustedSize * 1.1;
                  logger.info('Position size increased due to low ADL quantile (low liquidation risk)', {
                    context: 'AgentCoordinator',
                    data: {
                      symbol: workflow.symbol,
                      adlQuantile: adlQuantile.adlQuantile,
                ...
                logger.debug('ADL quantile check failed (non-blocking)', {
                  context: 'AgentCoordinator',
                  error: adlError instanceof Error ? adlError.message : String(adlError)
                });
              }
              
              // CRITICAL: Validate position size against leverage brackets
              // Per API docs: Position notional must fit within account brackets
              try {
                const leverageBracket = await asterDexService.getLeverageBracket(workflow.symbol);
                if (leverageBracket && leverageBracket.brackets && leverageBracket.brackets.length > 0) {
                  const positionValueFinal = adjustedSize * currentPrice;
                  const accountNotional = balance; // Use balance as account notional
                  
                  // Find appropriate bracket based on account notional
                  const appropriateBracket = leverageBracket.brackets.find((bracket: any) => 
                    accountNotional >= bracket.notionalFloor && accountNotional <= bracket.notionalCap
                  ) || leverageBracket.brackets[0];
                  
                  // Ensure leverage doesn't exceed bracket limit
                  if (optimalLeverage > appropriateBracket.initialLeverage) {
                    optimalLeverage = appropriateBracket.initialLeverage;
                    logger.warn('Leverage adjusted to bracket limit', {
                      context: 'AgentCoordinator',
                      data: {
                        requested: optimalLeverage,
                        bracketLimit: appropriateBracket.initialLeverage,
                        adjusted: optimalLeverage
                      }
                    });
                  }
                  
                  // Ensure position notional fits within bracket
                  if (positionValueFinal > appropriateBracket.notionalCap) {
                    adjustedSize = (appropriateBracket.notionalCap / currentPrice) * 0.95; // 95% of cap for safety
                    logger.warn('Position size adjusted to fit leverage bracket', {
                      context: 'AgentCoordinator',
                      data: {
                        originalSize: safePositionSize,
                        adjustedSize,
                        bracketCap: appropriateBracket.notionalCap,
                        positionValue: adjustedSize * currentPrice
                      }
                    });
                  }
                ...
                logger.debug('Leverage bracket check failed (non-blocking)', {
                  context: 'AgentCoordinator',
                  error: bracketError instanceof Error ? bracketError.message : String(bracketError)
                });
              }
              
              // Validate balance sufficiency
              const positionValueFinal = adjustedSize * currentPrice;
              const requiredMargin = positionValueFinal / optimalLeverage;
              const availableBalance = balanceConfig.availableBalance || 0;
              
              if (requiredMargin > availableBalance) {
                // Adjust position size to fit available balance
                const maxPositionValue = availableBalance * optimalLeverage;
                adjustedSize = Math.floor((maxPositionValue / currentPrice) / (stepSize || 0.0001)) * (stepSize || 0.0001);
                
                logger.warn('Position size adjusted due to insufficient balance', {
                  context: 'AgentCoordinator',
                  data: {
                    requiredMargin,
                    availableBalance,
              ...
              // Update position size with validated value
              safePositionSize = adjustedSize;
            }
          } catch (filterError) {
            logger.warn('Failed to validate position size against exchange filters (using calculated size)', {
              context: 'AgentCoordinator',
              error: filterError instanceof Error ? filterError.message : String(filterError)
            });
            // Continue with calculated size - asterDexService will validate on order placement
          }
          ...
          riskAssessment.leverage = optimalLeverage;
          riskAssessment.reasoning = `FORCE APPROVED: Chief Analyst ${finalDecision.action} with ${(finalDecision.confidence * 100).toFixed(0)}% confidence. Using ${(riskPercent * 100).toFixed(1)}% risk, ${optimalLeverage}x leverage${sizingReason}. Original rejection: ${riskAssessment.reasoning}`;
      ...
      // Audit rejections for learning
      if (!riskAssessment.approved && finalDecision?.action && finalDecision.action !== 'HOLD') {
        try {
          await recordTradeRejection({
            symbol: workflow.symbol,
            reason: typeof riskAssessment.reasoning === 'string' ? riskAssessment.reasoning : 'rejected',
            confidence: finalDecision.confidence,
            expectedValue: (finalDecision as any)?.expectedValue || undefined,
            style: (riskAssessment.riskRewardRatio && riskAssessment.riskRewardRatio < 2) ? 'MICRO' : 'MACRO'
          });
        } catch (rejErr) {
          logger.debug('Failed to record trade rejection (non-blocking)', {
            context: 'AgentCoordinator',
            data: { error: rejErr instanceof Error ? rejErr.message : String(rejErr) }
          });
        }
      }
...
Respond in JSON:
{
  ...
  "reasoning": "your execution strategy"
      ...
      try {
        ...
        try {
          ...
          const extracted = deepseekService.extractStructuredData(content);
          if (extracted && !extracted.error) {
            logger.warn('Using extracted data from malformed Execution Specialist response', {
              context: 'AgentCoordinator',
              data: { hasReadyToExecute: 'readyToExecute' in extracted }
            });
            executionPlan = extracted;
            // Ensure readyToExecute is set if missing
            if (executionPlan.readyToExecute === undefined) {
              executionPlan.readyToExecute = riskAssessment.approved;
            }
          } else {
  ...
  private async executeTradeExecution(workflow: TradingWorkflow): Promise<any> {
    ...
    try {
      ...
      if (!executionPlan?.readyToExecute || !riskAssessment?.approved) {
        ...
        logger.info(`[SKIP] Trade skipped for ${workflow.symbol} - ${reason}`, {
          context: 'AgentCoordinator',
          data: {
            symbol: workflow.symbol,
            approved: riskAssessment?.approved,
      ...
      // SLIPPAGE GUARD: abort execution if expected vs mid deviates too much
      try {
        const { wsMarketService } = await import('@/services/exchange/websocketMarketService');
        const microSig = wsMarketService.getMicrostructureSignal(workflow.symbol);
        const classifyStyle = (entry: number, stop: number, take: number): 'MICRO' | 'MACRO' => {
        ...
        const expected = finalDecision?.currentPrice || workflow.context.marketData?.price || 0;
        const priceData = wsMarketService.getMarkPriceDivergence(workflow.symbol);
        if (expected > 0 && priceData && priceData.lastPrice > 0) {
          const mid = priceData.lastPrice;
          const style = classifyStyle(
            expected,
            riskAssessment.stopLoss || expected,
            riskAssessment.takeProfit || expected
          );
          // Slightly relaxed to improve execution rate
          const allowed = style === 'MICRO' ? 0.004 : 0.008; // 40bps micro, 80bps macro
          const slip = Math.abs(mid - expected) / mid;
          if (slip > allowed) {
            logger.warn(`Trade execution skipped: slippage guard hit (${(slip * 100).toFixed(2)}% > ${(allowed * 100).toFixed(2)}%)`, {
              context: 'AgentCoordinator',
              data: { symbol: workflow.symbol, style, expected, mid }
            });
            agent.status = 'idle';
            agent.currentTask = undefined;
            agent.performance.totalTasks++;
            return {
              executed: false,
              reason: `Slippage guard: ${(slip * 100).toFixed(2)}% > ${(allowed * 100).toFixed(2)}%`
            };
          }
        }
      } catch (slipGuardError) {
        logger.debug('Slippage guard check failed (non-blocking)', {
          context: 'AgentCoordinator',
          error: slipGuardError instanceof Error ? slipGuardError.message : String(slipGuardError)
        });
      }
      ...
      const { asterDexService } = await import('@/services/exchange/asterDexService');
      const { positionMonitorService } = await import('@/services/trading/positionMonitorService');
      const { TRADING_CONSTANTS } = await import('@/constants');
      
      // CRITICAL FIX: Cap position size to MAX_ORDER_SIZE to avoid exchange rejection
      const originalPositionSize = riskAssessment.positionSize;
      if (riskAssessment.positionSize > TRADING_CONSTANTS.MAX_ORDER_SIZE) {
        riskAssessment.positionSize = TRADING_CONSTANTS.MAX_ORDER_SIZE;
        logger.warn('Position size capped to MAX_ORDER_SIZE', {
          context: 'AgentCoordinator',
          data: {
            symbol: workflow.symbol,
            originalSize: originalPositionSize,
            cappedSize: riskAssessment.positionSize,
            maxAllowed: TRADING_CONSTANTS.MAX_ORDER_SIZE
          }
        });
      }
      ...
      while (attempt < maxRetries && !tradeResult) {
        ...
        try {
          ...
          logger.info(`[EXECUTE] EXECUTING TRADE (attempt ${attempt}/${maxRetries}, ${orderType})`, {
            context: 'AgentCoordinator',
            data: {
              symbol: workflow.symbol,
              action: riskAssessment.action,
          ...
          // ENTERPRISE: Prepare ML training data for simulation mode
          const marketDataForML = {
            ...(workflow.context.marketData as any),
            stopLoss: riskAssessment.stopLoss,
            takeProfit: riskAssessment.takeProfit,
            score: (finalDecision as any)?.opportunityScore || (workflow.context.technicalAnalysis as any)?.score || 0,
            regime: (workflow.context.marketData as any)?.regime || (workflow.context.technicalAnalysis as any)?.marketRegime || 'unknown'
          };

          const aiDecisionForML = {
            ...finalDecision,
            technicalScore: (workflow.context.technicalAnalysis as any)?.score || 0,
            chiefConfidence: finalDecision?.confidence || 0,
            riskApproved: riskAssessment.approved,
            riskReason: typeof riskAssessment.reasoning === 'string' ? riskAssessment.reasoning : '',
            opportunityScore: (finalDecision as any)?.opportunityScore || 0
          };
          ...
          if (tradeResult && finalDecision?.currentPrice) {
            ...
            if (slippage > 0.2) {
              logger.warn('[WARN] High slippage detected', {
                context: 'AgentCoordinator',
                data: {
                  symbol: workflow.symbol,
          ...
          if (!useLimitOrder && attempt < maxRetries) {
            logger.info('[FALLBACK] Falling back to LIMIT order for better execution', {
              context: 'AgentCoordinator',
      ...
      if (!tradeResult || !tradeResult.orderId) {
        logger.error('[ERROR] Trade execution returned invalid result', {
          context: 'AgentCoordinator',
          data: {
            symbol: workflow.symbol,
      ...
      logger.info('[OK] Trade executed successfully', {
        ...
        try {
          ...
          const tradeEntry = {
            ...
            pnlPercent: 0,
            leverage: riskAssessment.leverage ?? 10, // Ensure leverage is defined (default to 10 if missing)
            entryReason: `Multi-Agent Analysis: ${typeof finalDecision?.reasoning === 'string' 
              ? finalDecision.reasoning.substring(0, 200) 
              : Array.isArray(finalDecision?.reasoning) 
                ? finalDecision.reasoning.join(' ').substring(0, 200)
                : 'High confidence opportunity'}`,
            entryConfidence: (finalDecision?.confidence || 0) * 100, // Store as percentage (0-100) for consistency with DB
            entrySignals: {
              volumeScore: (workflow.context.marketData as any)?.volumeScore || (workflow.context.marketData as any)?.volumeRatio || 0,
              momentumScore: (workflow.context.marketData as any)?.momentumScore || (workflow.context.marketData as any)?.momentum || 0,
              liquidityScore: (workflow.context.marketData as any)?.liquidityScore || (workflow.context.marketData as any)?.liquidity || 0,
              rsi: (workflow.context.marketData as any)?.rsi || (workflow.context.technicalAnalysis as any)?.rsi || 0,
              volatility: (workflow.context.marketData as any)?.volatility || 0,
              technicalSignals: (workflow.context.technicalAnalysis as any)?.signals || [],
              chiefDecision: finalDecision?.action || 'HOLD',
              chiefConfidence: finalDecision?.confidence || 0,
              riskRewardRatio: riskAssessment.riskRewardRatio || 0,
              sentiment: (workflow.context.sentimentData as any)?.sentiment || 'neutral'
            },
            entryMarketRegime: (workflow.context.marketData as any)?.regime || (workflow.context.technicalAnalysis as any)?.marketRegime || 'unknown',
            entryScore: (finalDecision as any)?.opportunityScore || (workflow.context.technicalAnalysis as any)?.score || 0,
        ...
        // Record trade for goal tracking
        try {
          goalTracker.recordTrade({
            symbol: workflow.symbol,
            side: riskAssessment.action === 'BUY' ? 'LONG' : 'SHORT',
            entryPrice: finalDecision?.currentPrice || 0,
            size: riskAssessment.positionSize,
          ...
          logger.info('Trade recorded for goal tracking', {
            context: 'AgentCoordinator',
            data: { symbol: workflow.symbol }
          });
        } catch (goalError) {
          logger.warn('Failed to record trade for goal tracking (non-critical)', {
            context: 'AgentCoordinator',
            data: { error: goalError }
          });
        }
        ...
        try {
          ...
          // CRITICAL FIX: Handle case where tradeResult.price is 0 (falsy but not undefined)
          const entryPrice = finalDecision?.currentPrice || 
                             (tradeResult.price && tradeResult.price > 0 ? tradeResult.price : null) || 
                             workflow.context.marketData?.price || 
                             0;
          if (entryPrice <= 0) {
          ...
          const positionId = await positionMonitorService.addPosition({
            ...
            size: riskAssessment.positionSize,
            leverage: riskAssessment.leverage || 10, // Default to 10x if not specified
            stopLoss: riskAssessment.stopLoss,
            takeProfit: riskAssessment.takeProfit,
          ...
          logger.info('[OK] Position added to monitor successfully!', {
            context: 'AgentCoordinator',
            data: { 
              positionId, 
              symbol: workflow.symbol,
          ...
          // Record trade entry rationale (DB optional)
          try {
            // CRITICAL FIX: Ensure all required fields for lessons learned are populated
            const marketData = workflow.context.marketData as any;
            const technicalAnalysis = workflow.context.technicalAnalysis as any;
            
            // Build comprehensive entrySignals object for pattern analysis
            const entrySignals = {
              volumeScore: marketData?.volumeScore || marketData?.volumeRatio || 0,
              momentumScore: marketData?.momentumScore || marketData?.momentum || 0,
              liquidityScore: marketData?.liquidityScore || marketData?.liquidity || 0,
              rsi: marketData?.rsi || technicalAnalysis?.rsi || 0,
            ...
            await recordTradeEntry({
              id: tradeResult.orderId || positionId,
              timestamp: Date.now(),
              symbol: workflow.symbol.replace('/', ''),
              side: side === 'LONG' ? 'LONG' : 'SHORT',
              size: riskAssessment.positionSize,
              entryPrice,
              leverage: riskAssessment.leverage || 10, // Default to 10x if not specified
              entryReason: typeof finalDecision?.reasoning === 'string' 
                ? finalDecision.reasoning 
                : Array.isArray(finalDecision?.reasoning) 
                  ? finalDecision.reasoning.join(' ')
                  : finalDecision?.action || 'trade',
              entryConfidence: (finalDecision?.confidence || 0) * 100, // Store as percentage (0-100) for consistency
              entrySignals: entrySignals, // Comprehensive signals object
              entryMarketRegime: marketData?.regime || technicalAnalysis?.marketRegime || 'unknown',
              entryScore: (finalDecision as any)?.opportunityScore || technicalAnalysis?.score || 0,
              takeProfit: riskAssessment.takeProfit,
              stopLoss: riskAssessment.stopLoss,
              style: (riskAssessment.riskRewardRatio && riskAssessment.riskRewardRatio < 2) ? 'MICRO' : 'MACRO'
            });
          } catch (entryErr) {
            logger.debug('Trade entry record failed (non-blocking)', {
              context: 'AgentCoordinator',
              data: { error: entryErr instanceof Error ? entryErr.message : String(entryErr) }
            });
          }
          ...
          if (!addedPosition) {
            logger.error('[ERROR] Position was not found after adding to monitor', {
              context: 'AgentCoordinator',
              data: {
                positionId,
                symbol: workflow.symbol,
  ...
  private trackWorkflowPerformance(symbol: string, duration: number, success: boolean, stepsCompleted: number): void {
    ...
    if (existing.totalExecutions % 10 === 0) {
      logger.info('[METRICS] Workflow Performance Metrics', {
        context: 'AgentCoordinator',
        data: {
          symbol,

...
```mermaid
graph TB
    subgraph PRESENTATION["PRESENTATION LAYER<br/><i>React 18 + Next.js 14 + Framer Motion</i>"]
        P1[Dashboard]
        P2[Charts]
        P3[Positions]
        P4[AI Chat]
    ...
    subgraph API["API LAYER<br/><i>Next.js API Routes</i>"]
        A1["/api/startup"]
        A2["/api/trading-status"]
        A3["/api/realtime-market"]
        A4["/api/..."]
    end

    subgraph DATA["DATA AGGREGATION LAYER<br/><i>Unified Data Aggregator</i>"]
        D1[WebSocket Streams]
        D2[Funding Rates]
        D3[Liquidation Monitor]
        D4[Order Book Depth]
    end
    ...
    subgraph AGENTS["MULTI-AGENT COORDINATOR<br/><i>DeepSeek R1 — Groq or Ollama</i>"]
        TA[Technical Analyst] --> CA[Chief Analyst] --> RM[Risk Manager] --> ES[Execution Specialist]
    end

    subgraph EXEC["EXECUTION LAYER"]
        E1[Aster DEX Service]
        E2[Position Monitor]
        E3[Chandelier Exit]
        E4[Performance Tracker]
    end

    subgraph PERSIST["PERSISTENCE LAYER<br/><i>PostgreSQL — Supabase/Neon</i>"]
        DB[(trades | open_positions | ml_training_data | ai_messages)]
    end
...
```mermaid
graph TD
    S["1. MARKET SCANNER<br/><i>Top 100 symbols by volume<br/>Filters: volume > 1.5x avg, spread < 0.5%, liquidity</i>"]
    T["2. TECHNICAL ANALYST<br/><i>DeepSeek R1</i><br/><i>Multi-timeframe analysis (1m, 5m, 15m, 1h, 4h)<br/>Pattern recognition, support/resistance</i>"]
    C["3. CHIEF ANALYST<br/><i>DeepSeek R1</i><br/><i>Synthesizes all data<br/>BUY / SELL / HOLD decision with confidence</i>"]
    R["4. RISK MANAGER<br/><i>DeepSeek R1</i><br/><i>Kelly Criterion position sizing<br/>ATR-based stop-loss/take-profit<br/>Portfolio risk validation</i>"]
    E["5. EXECUTION SPECIALIST<br/><i>DeepSeek R1</i><br/><i>Optimal order timing<br/>Slippage protection</i>"]
    M["6. POSITION MONITOR<br/><i>24/7</i><br/><i>Chandelier Exit trailing stops<br/>Stop-loss/take-profit monitoring<br/>Timeout exits (24 hours max)</i>"]
